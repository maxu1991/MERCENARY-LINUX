...
To simplify the installation process we added a command "merc_install" that completes the entire setup process for you.  So download the .iso, install bare metal or create a virtual machine <strong>(follow instructions from hunttools.org's wiki)</strong>, boot up and run sudo merc_install and follow the instructions on the screen. We hope you enjoy! 
